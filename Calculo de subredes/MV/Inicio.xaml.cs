    ...
    public partial class Inicio : ContentPage
    {
        ...
        private async void OnCopiarInformacionClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    await DisplayAlert("Error", "No hay información generada para copiar.", "OK");
                    return;
                }

                await Clipboard.SetTextAsync(textoCopiar);
                await DisplayAlert("Éxito", "La información ha sido copiada al portapapeles.", "OK");
            }
            catch (Exception ex)
            {
                await DisplayAlert("Error", $"No se pudo copiar la información: {ex.Message}", "OK");
            }
        }
    }
}
