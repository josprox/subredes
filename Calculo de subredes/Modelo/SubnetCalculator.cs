...
public class SubnetCalculator
{
    ...
    public static int CalcularPrefijoDesdeMascara(string mascara)
    {
        return mascara.Split('.').Select(octeto => Convert.ToString(int.Parse(octeto), 2).Count(c => c == '1')).Sum();
    }
    ...
    public static Dictionary<string, string> CalcularSubredes(string ipBase, int bitsSubred, int numSubred, int? prefijoClase = null)
    {
        try
        {
            ...
            int prefijoCalculado = CalcularPrefijoDesdeMascara(mascaraSubred.ToString());
            ...
            return new Dictionary<string, string>
            {
                ...
                {"Prefijo Calculado desde la Máscara", $"/{prefijoCalculado}"},
